...
$user = $userController->getUserById($user_id); // Supposons que vous ayez une méthode pour récupérer l'utilisateur par ID

// Si l'utilisateur n'est pas trouvé, rediriger vers la page de profil
if (!$user) {
    header("Location: profile.php");
    exit();
}

// Initialiser les variables de l'utilisateur
$firstname = $user['firstname'];
$lastname = $user['lastname'];
$email = $user['email'];
$role = $user['role'];

$error = ""; // Pour afficher des erreurs
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $newRole = $_POST['role']; // Si le rôle peut être modifié
    $newPassword = $_POST['password']; // Optionnel, si l'utilisateur veut changer son mot de passe

    // Appeler la méthode pour mettre à jour les informations de l'utilisateur
    $updateResult = $userController->updateProfile($user_id, $newFirstname, $newLastname, $newEmail, $newRole, $newPassword);
    ...
    if ($updateResult === "Profile updated successfully!") {
        ...
        $error = $updateResult; // Message d'erreur retourné par la méthode updateProfile()
    }
}
   ...
   <header>
      <div class="header">
                     ...
                     <div class="header-lang-item tp-header-setting">
                        ...
                        <ul>
                           <li>
                              <a class="furniture-clr-hover" href="profile.php">My Profile</a>
                           </li>
                           <li>
...
<script>
    function validateForm() {
        ...
        if (!/^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/.test(firstname)) {
            alert('Le prénom ne doit contenir que des lettres.');
            return false;
        }

        // Validation du nom
        if (!/^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/.test(lastname)) {
