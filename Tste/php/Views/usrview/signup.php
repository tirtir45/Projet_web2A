...
if (
    ...
    if (
        ...
        // Création de l'objet User avec les informations du formulaire
        $user = new User(
            null,  // L'ID peut être null, il sera généré automatiquement par la base de données
            $_POST['firstname'],
            $_POST['lastname'],
            $_POST['gender'], 
            $_POST['email'],
            $_POST['password'],  // Notez que vous pouvez hacher le mot de passe ici si nécessaire
            "user"  // Role par défaut : "user"
        );

        // Appel de la méthode addUser() du contrôleur pour enregistrer l'utilisateur
        $userU->register($user);
...
<script>
    function validateForm() {
        ...
        <p class="text-center mt-3">
            Already have an account? <a href="#" class="text-muted">Log In</a>
        </p>
    </div>
</div>
<!-- Sign Up Section End -->
