...
$typeC = new TypeC();
$produits = $produitC->listeProduits();
...
<body>
                  ...
                  <form action="#">
                    ...
                    <button type="submit">
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M13.4443 13.4445L16.9999 17" stroke="white" stroke-width="2" stroke-linecap="round"
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-lg-6 col-md-10">
            ...
            <form action="../ajout_produit.php" method="POST">
              ...
              <div class="form-group mb-3">
                <label for="id ">Non de Produit</label>
                <input type="text" class="form-control" id="name" name="name"
                  placeholder="Entrez le nom du produit" />
              </div>
              <input class="" type="hidden" id="id" name="id" value="<?php echo uniqid(); ?>">
              <!-- Type de produit -->
              <div class="form-group mb-3">
                <label for="type">Taille du Produit</label>
              ...
              <div class="form-group mb-3">
                ...
                <select id="category" name="category" onchange="updateHiddenInputs()">
                  <?php
                  // Loop through the types and create options
                  foreach ($types as $type) {
                    // Preselect the matching category if needed
                    echo "<option value='{$type['type_produit']}' data-id='{$type['id']}'>{$type['type_produit']}</option>";
                  }
                  ?>
                </select>

                <!-- Hidden inputs for id and name -->
                <input type="hidden" id="category_id" name="typeId" value="<?php echo $oldproduit['type_id'] ?? ''; ?>">
                <input type="hidden" id="category_name" name="type_produit" value="<?php echo $oldproduit['type_produit'] ?? ''; ?>">
                <!-- Hidden inputs for id and name -->
                <input type="hidden" id="category_id" name="typeId" value="<?php echo $oldproduit['type_id'] ?? ''; ?>">
                <input type="hidden" id="category_name" name="type_produit" value="<?php echo $oldproduit['type_produit'] ?? ''; ?>">
              </div>
          ...
          <div class="col-lg-6 col-md-10 mt-5 mt-lg-0">
            <h2 class="text-center mb-4">Liste des Categorys</h2>
            <table class="table table-bordered">
              ...
              <tbody>
                <a href="type.php" class="btn btn-success btn-sm">Ajouter
                  Type</a>
  ...
  <script>
    // JavaScript function to update the hidden inputs
    function updateHiddenInputs() {
      var select = document.getElementById("category");
      var selectedOption = select.options[select.selectedIndex];

      // Get the selected values
      var selectedTypeId = selectedOption.getAttribute("data-id");
      var selectedTypeName = selectedOption.value;
      console.log(selectedTypeId, selectedTypeName)
      // Update the hidden inputs
      document.getElementById("category_id").value = selectedTypeId;
      document.getElementById("category_name").value = selectedTypeName;
    }

    // Call the function to set the initial values if needed (on page load)
